...
def createMotifSequences(df, fasta_ref):
    ...
    for index, row in df.iterrows():
        ...
        alt_list.append(alt_z)
        ref_list.append(ref_z)
        ...
        rev_alt_list.append(alt_z)
        rev_ref_list.append(ref_z)
...
def tf_score(model, ref_list, alt_list, rev_alt_list, rev_ref_list):
    ...
    alt_list = np.vstack(alt_list)
    rev_alt_list = np.vstack(rev_alt_list)
    rev_ref_list = np.vstack(rev_ref_list)
    ...
    for idx in range(1, len(batches_ref)):
        ...
        jsd_score.append(probs_jsd_diff) 
        score.append(altpred_cts - refpred_cts)
        ...
        rev_jsd_score.append(probs_jsd_diff) 
        rev_score.append(altpred_cts - refpred_cts)
